...
public class MoveSpawn
{
  public MoveSpawn()
  {
    new Terminal.ConsoleCommand("move_spawn", "[x,z,y] Moves the default spawn point to the given coordinates (player's current coordinates if not given).", static (args) =>
    {
      ...
      if (ZNet.instance.IsServer())
      {
        ...
        var location = zs.m_locationInstances.First(static location => Helper.IsValid(location.Value.m_location) && location.Value.m_location.m_prefab.Name == "StartTemple");
    ...
    AutoComplete.Register("move_spawn", static (int index, int subIndex) =>
    {
