...
namespace ServerDevcommands {
  ...
  public class NoMapCommand {
    public NoMapCommand() {
      new Terminal.ConsoleCommand("nomap", "[set] [server = 1] - Toggles or sets the nomap mode for the server. If server is 0, the mode is only changed for the character.", delegate (Terminal.ConsoleEventArgs args) {
        var set = false;
        if (args.Length < 2)
